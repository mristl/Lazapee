...
def quick_sort(arr, key_func):
    ...
    pivot = arr[0]
    left = [x for x in arr[1:] if key_func(x) <= key_func(pivot)]
...
def view_books(request):
    ...
    if sort_field == 'title':
        borrowed_books = quick_sort(borrowed_books, key_func=lambda book: book.title)
    elif sort_field == 'author':
        borrowed_books = quick_sort(borrowed_books, key_func=lambda book: book.author)
    elif sort_field == 'publisher':
        borrowed_books = quick_sort(borrowed_books, key_func=lambda book: book.publisher)
    elif sort_field == 'days_remaining':
        borrowed_books = quick_sort(borrowed_books, key_func=lambda book: book.days_remaining)
    elif sort_field == 'accession_number':
        borrowed_books = quick_sort(borrowed_books, key_func=lambda book: book.accession_number)
