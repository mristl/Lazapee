...
def merge_sort(arr, key_func): #sorting criteria
    ...
    left_half = merge_sort(arr[:mid], key_func) #create left half by slicing, then sorted
    right_half = merge_sort(arr[mid:], key_func)

    return merge(left_half, right_half, key_func) #after both halves have been sorted
...
def merge(left, right, key_func):
    ...
    sorted_list.extend(left) #add remaining elements
...
def view_books(request):
    ...
    if search_query:
        ...
        borrowed_books,
        search_query,
        key=lambda book: f"{book.title} {book.author} {book.publisher} {book.user.name}"
    )
